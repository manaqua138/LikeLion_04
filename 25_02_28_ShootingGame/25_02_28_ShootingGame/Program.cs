...
namespace ShootingGame
{
    ...
    public class Player
    {
        ...
        public void GameOver(ref Enemy enemy)
        {
            Console.Clear();
            Console.SetCursorPosition(25, 10);
            Console.Write($"게임 오버! 점수는 {Score} 입니다.");
            Console.SetCursorPosition(25, 12);
            Console.Write($"엔터를 눌러 재시작하세요");
            Console.ReadLine();
            playerLife = 3;
            Score = 100;
            itemCount = 0;
        ...
        public void ClashEnemyAndBullet(Enemy enemy)
        {
            ...
            for (int i = 0; i < 20; i++)
            {
                ...
                if (playerBullet[i].fire == true)
                {
                    ...
                    if (playerBullet[i].y == enemy.enemyY + 1 && itemCount >= 1)
                    {
                        if (playerBullet[i].x >= (enemy.enemyX - 1)
                            && playerBullet[i].x <= (enemy.enemyX + 1)) //충돌
                        {
                    ...
                    if (playerBullet[i].y == enemy.enemyY - 1 && itemCount >= 2)
                    {
                        if (playerBullet[i].x >= (enemy.enemyX - 1)
                            && playerBullet[i].x <= (enemy.enemyX + 1)) //충돌
                        {
    ...
    public class Enemy //적 클래스
    {
        ...
        public void EnmeyMove(ref Player player)
        {
            ...
            while (true) //무한반복
            {
                ...
                if (dwTime + 50 < Environment.TickCount)
                {
                    ...
                    if (player.playerLife > 0)
                    {
                        ...
                        enemy.EnmeyMove(ref player);//적이동
                        enemy.EnemyDraw();//적그리기
                                          //충돌처리
                        player.ClashEnemyAndBullet(enemy);
                    }
                    else
                        player.GameOver(ref enemy);
                }
            }
